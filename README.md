...
(gpt-4o-transcribe) → DeepInfra (Voxtral)**. First success wins; a down or
unavailable rung is skipped transparently instead of failing the whole transcription.
...
4. **DeepInfra**, Voxtral — last resort.

Reorder `_LADDER` in `provider.py` if your own cost/quality priorities differ.

## Language handling

All 4 rungs auto-detect rather than force a language. Only xAI's underlying handler
...
`DEEPINFRA_API_KEY` you have — a rung with no credentials is skipped, not an error, as
long as at least one rung further down the ladder has one.
...
`_transcribe_deepinfra` functions rather than reimplementing each provider's HTTP/auth/retry
